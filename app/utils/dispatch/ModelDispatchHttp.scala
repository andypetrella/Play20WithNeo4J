...
class ModelHandlers(subject: HandlerVerbs) {
  ...
  def >^>[M <: Model, T](block: (M) => T)(implicit fmt: Format[M], filter: (JsValue) => JsValue = (j: JsValue) => j) = new PlayJsonHandlers(subject) >! {
    (jsValue) =>
      block(fromJson[M](filter(jsValue)))
  }

  def >^*>[M <: Model, T](block: (Iterable[M]) => T)(implicit fmt: Format[M], filter: (JsValue) => Iterable[JsValue] = (j: JsValue) => Seq(j)) = new PlayJsonHandlers(subject) >! {
    (jsValue) =>
